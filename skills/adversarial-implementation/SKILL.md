...
description: >
  Automated adversarial implementation framework using Claude Code Agent Teams. Takes a final-plan.md
  from the adversarial-proposal skill and produces a vetted implementation document with exact code
  diffs, test specifications, and a coverage matrix proving every plan item is addressed. Two agents
  work independently — a Surgical Implementer (minimal diffs) and a Coverage Guardian (complete tests)
  — then a Synthesizer merges them, both critique, and the Synthesizer reconciles into a final
  implementation.md. Use this skill after adversarial-proposal completes, or whenever the user wants
  to create an implementation document from a plan. Trigger on phrases like "create implementation",
  "implement the plan", "adversarial implementation", or "implementation doc".
---
