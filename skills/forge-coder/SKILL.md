...
5. **Pre-flight diff validation** — for every modification step (not new files):
   ...
   - Record: FOUND (with line number) or NOT FOUND
...
6. Report pre-flight results:
   ...
   Step 4: src/foo/qux.py — NOT FOUND

   Result: 1 mismatch. STOPPING.
   Mismatched step 4: old_string not found in src/foo/qux.py
   The file may have changed since implementation.md was written.
   Re-run adversarial-implementation to produce updated diffs.
   ```

7. If ANY step has a mismatch: stop and report. Write a partial coder-report with
...
signaling (e.g., reporting completion to an orchestrator). This skill
focuses solely on executing the implementation and producing the report.
