...
1. **Diff Correctness**
   ...
   - Are the `old_string` values unique enough for a mechanical edit pass?
   - Are the `new_string` values syntactically valid?
   - If a step creates a new file, does it use the canonical shape (`Action: new_file`, `old_string = # new file`, full file content in `new_string`)?
...
6. Is mechanically executable by `$forge-coder` without guesswork or repair
