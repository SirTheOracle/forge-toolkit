...
4. **Diff**: `old_string` and `new_string` — exact, copy-pasteable
   - For modified files, `old_string` must be unique in the current source
...
- Reject standalone `# new_file` blocks, prose placeholders, and partial snippets for new files
- Ensure each commit-group test command is runnable after that group's steps and does not depend on files created later
