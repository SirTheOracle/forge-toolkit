...
3. **For each plan step**, produce the exact `old_string -> new_string` diff:
   - One step, one file, one diff pair
   - Include enough surrounding context in `old_string` to be unambiguous
   - Make `old_string` unique in the target file, not just present
   - The `new_string` must be syntactically correct and complete
   - For new files, use `**Action**: new_file`, set `old_string` to the literal text `# new file`, and put the full file contents in `new_string`
   - Never emit a standalone `# new_file` block or a partial-file placeholder
...
6. **Write a verification test** for each step or group — how do you prove this step worked?
   - Ensure each commit-group test command is runnable after that group's steps alone

## What Makes a Good Implementation Doc

- Every plan item maps to at least one exact diff
- Every diff is parser-safe for `$forge-coder`, not just readable to a human
...
- Using placeholders like `...`, `rest of file unchanged`, or a lone `# new_file` marker
