...
- Preserve unrelated user/worktree changes; never use destructive Git recovery.
- Use project-local test commands from project config where available.
- If a necessary command is denied by the runtime sandbox, report the typed block; never convert it into ambient or broader authority.

## Required Reads

Before any code action:

1. Project config, first existing path: `.codex/forge-project.yml`, `.claude/forge-project.yml`, `~/.codex/forge-project.yml`, then `~/.claude/forge-project.yml`
...
7. `~/.codex/skills/fix-coder/references/fix-diffs-format.md` — fix-diffs.md template
8. `~/.codex/skills/fix-coder/references/fix-coder-report-format.md` — final report template
...
**Token mapping note.** The substitution tokens used here (`{{test_command}}`, `{{lint_command}}`, `{{type_check_command}}`, `{{activate_venv}}`, `{{backend_working_dir}}`, etc.) are identical to forge-coder Phase 0. See `~/.codex/skills/forge-coder/SKILL.md` for the full token table; this skill resolves them the same way.
