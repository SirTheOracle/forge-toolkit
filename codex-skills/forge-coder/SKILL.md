...
3. **If tests PASS:**
   - Write the group message to `.dev/forge-tmp/commit-message.txt` and capture
     `expected_head` from the current physical worktree.
   - Invoke `forge-git-request commit` with `--root "$PROJECT_ROOT"`,
     `--expected-head "$expected_head"`, the message file, `--`, and every group
     path as a separate literal argument. Direct Git mutation is forbidden.
   - On a typed broker conflict, record the exact code, expected/current HEAD,
     and submitted path list in `coder-report.md`, callback `BLOCKED`, and do not retry.
