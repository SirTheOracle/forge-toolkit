...
For Codex, treat the tool names in this document as an upstream protocol reference rather than a literal API. Map `TeamCreate`, `Agent`, and `SendMessage` to the closest Codex delegation primitives, or use `references/subagent-fallback.md` when file-based coordination is the better fit.
...
- **If Agent Teams is NOT available, or you are running the Codex copy**: Use the subagent fallback or the equivalent Codex subagent workflow
