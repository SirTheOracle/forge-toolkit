...
- What is the real workflow gate for this feature?
- Did this report execute the gate, or is the gate blocked?

### 1.5. Real Workflow Gate

This section is mandatory. Targeted unit tests, mocked fixtures, helper tests, route-only tests, and source inspection are not enough for a clean QA result.

```markdown
### Real Workflow Gate: [name]

**Status**: PASS | FAIL | BLOCKED
**Path Type**: Browser UI | Authenticated API | Background job | Service command
**Real Inputs Used**: tenant_id=..., candidate_id=..., draft_id=..., job_id=..., URL=...
**Command / Steps**:
1. ...
2. ...
...
**Evidence saved**:
- HTTP response: `evidence/api-responses/C-A01-live-gate.json`
- Test output: `evidence/test-results/C-A01-live-gate.txt`
- Screenshot: `evidence/screenshots/C-A01-live-gate.png`
```

If the gate is `BLOCKED`, also create a finding with category `blocked-real-workflow`. A report with a blocked gate cannot recommend `SHIP IT`.
...
For wizard flows and multi-step forms, include the expected action container in the notes when relevant, for example: "Verified `Save Module 2` is visible inside `[data-testid=\"wizard-nav\"]`, not just somewhere on the page."
...
For wizard or stepper flows, call out discoverability coverage explicitly:
- Did each step have a visible primary action in the expected nav container?
- Were Playwright assertions scoped to that container rather than page-wide selectors?
...
- Use `SHIP IT` only when the real workflow gate passed and no high-severity defects remain.
- Use `FIX FIRST` when the gate failed in the product path.
- Use `NEEDS REAL-WORKFLOW TESTING` when the gate was not executed, even if all fixture/unit tests passed.
- Use `BLOCKED` when the environment prevents the gate from running.
